...
const JournalEntryPopup: React.FC<Props> = ({ entry, onClose, onDelete }) => {
  ...
  const dateVisited = new Date(entry.dateVisited).toLocaleDateString(undefined, {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
  });
        ...
        <Card.Body className="py-2 px-3">
          <Stack direction="horizontal" className="justify-content-between align-items-start">
            <div>
              ...
              <Card.Title as="h6" className="mb-0" style={{ marginTop: '10px' }}>
                {entry.title}
              </Card.Title>
            </div>
          ...
          <hr className="my-2" />
          <Card.Text style={{ whiteSpace: 'pre-wrap', fontSize: '14px' }}>{entry.body}</Card.Text>
